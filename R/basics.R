# for more info about the PubChem API: https://pubchemdocs.ncbi.nlm.nih.gov/pug-rest

#' GetNameUrlPugRest
#' Get the url for Pug Rest APIs with checmial name input
#'
#' @param chemName name of the chemical
#' @param searcBasehUrl basic search url
#' @param type type of the return file
#'
#' @return searchUrl in string
#' @export
#'
#' @examples GetNameUrlPugRest(chemName = "amoxicillin")
#'
GetNameUrlPugRest <- function(chemName, searcBasehUrl = "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/",type="/JSON"){
  searchUrl <- paste0(searcBasehUrl, chemName,type)
  return(searchUrl)
}
...
#' @export
#'
#' @examples GetPubRestJson(chemName = "amoxicillin")
#'
#' @import rjson
#'
GetPubRestJson <- function(chemName, waitTime = 0){
  link <- GetNameUrlPugRest(chemName)
...
GetAPIlink <- function(baseUrl = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/",
  ...
  paras <- paste0(stats::na.omit(c(db, id, apiKey,email,retmode,term,reldate,datetype,retmax,usehistory,retstart,tool,WebEnv,cmd)), collapse = "&")
